...
export function PublicPageRenderer({ data }: PublicPageRendererProps) {
  ...
  const primaryLink =
    enabledLinks.find((l) => l.variant === "primary" && l.type !== "divider") ??
    null;
        ...
        >
          <main className="mx-auto max-w-[480px] px-4 py-10">
            ...
            <header className="mb-6 flex flex-col items-center text-center">
              ...
              {/* Name */}
              <h1
                className={`public-title font-bold leading-snug text-[color:var(--header-title-color,#111827)] ${isLargeTitle ? "text-[26px]" : "text-[20px]"}`}
              >
                {profile.name}
              ...
              {/* CRP badge */}
              {profile.crp && (
                <p className="mt-1 text-[13px] font-medium text-[color:var(--header-meta-color,#4B5563)]">
                  {profile.crp}
                </p>
              )}
              ...
              {profile.subtitle && (
                <p className="mt-1.5 text-[13px] font-medium text-[color:var(--header-subtitle-color,#374151)]">
                  {profile.subtitle}
                </p>
              )}
              ...
              {/* Bio */}
              {profile.bio && (
                <p className="mt-2 max-w-[320px] text-[13px] leading-relaxed text-[color:var(--header-meta-color,#4B5563)]">
                  {profile.bio}
                </p>
              )}
            </header>
            ...
            <footer className="mt-8 flex items-center justify-center gap-5">
              ...
              {profile.instagram_url && (
                <a
                  ...
                  className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-[#F58529] via-[#DD2A7B] to-[#8134AF] text-white shadow-sm transition-transform hover:scale-105 hover:brightness-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-pink-300 focus-visible:ring-offset-2 focus-visible:ring-offset-[#F5F5F5]"
                >
                  <svg
                    viewBox="0 0 24 24"
                    className="h-5 w-5"
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            sameAs: [profile.instagram_url, profile.website_url].filter(
              Boolean,
            ),
          }),
        }}
